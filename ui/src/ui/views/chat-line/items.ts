import { extractTextCached } from "../../chat-claw/message-extract.ts";
import {
  isToolResultMessage,
  normalizeMessage,
  normalizeRoleForGrouping,
} from "../../chat-claw/message-normalizer.ts";
import { messageMatchesSearchQuery } from "../../chat-claw/search-match.ts";
import { extractToolCards, extractToolPreview } from "../../chat-claw/tool-cards.ts";
import type { ChatItem, MessageGroup, ToolCard } from "../../types/chat-types.ts";
import type { ChatProps } from "./types.ts";
import {
  chatViewState,
  getExpandedToolCards,
  getInitializedToolCards,
  getLastAutoExpandPref,
  setLastAutoExpandPref,
} from "./state.ts";
...
export function extractChatMessagePreview(toolMessage: unknown): {
  preview: Extract<NonNullable<ToolCard["preview"]>, { kind: "canvas" }>;
  text: string | null;
...
export function syncToolCardExpansionState(
  ...
  const previousAutoExpand = getLastAutoExpandPref(sessionKey);
  ...
  setLastAutoExpandPref(sessionKey, autoExpandToolCalls);
}
...
export function buildChatItems(props: ChatProps): Array<ChatItem | MessageGroup> {
  ...
  for (let i = historyStart; i < history.length; i++) {
    ...
    if (
      chatViewState.searchOpen &&
      chatViewState.searchQuery.trim() &&
      !messageMatchesSearchQuery(msg, chatViewState.searchQuery)
    ) {
      continue;
    }
